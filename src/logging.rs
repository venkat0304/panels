...
pub(crate) fn init_file_logging(file_name: &str) {
    ...
    let filter =
        EnvFilter::try_from_env("PANELS_LOG").unwrap_or_else(|_| EnvFilter::new("panels=info"));
...
pub(crate) fn help_log_paths_summary() -> String {
    ...
    format!(
        "{} (plus panels-client.log, panels-server.log)",
        dir.join("panels.log").display()
    )
}
...
mod tests {
    ...
    fn temp_log_path(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(unique).join("panels.log")
    }

    #[test]
    fn rotated_log_path_appends_numeric_suffix() {
        let path = PathBuf::from("/tmp/panels.log");
        assert_eq!(
            rotated_log_path(&path, 2),
            PathBuf::from("/tmp/panels.log.2")
        );
    }
    ...
    fn write_replaces_log_without_retained_files_when_size_limit_is_reached() {
        ...
        let writer = RotatingFileMakeWriter::new(dir.clone(), "panels.log", 8, 0).unwrap();
        {
