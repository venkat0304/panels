...
fn apply_pane_terminal_env(cmd: &mut CommandBuilder) {
    // Each pane is rendered by panels's own terminal layer, not the outer terminal
...
impl PaneRuntime {
    ...
    pub fn spawn(
        ...
        cmd.cwd(cwd);
        cmd.env(crate::PANELS_ENV_VAR, crate::PANELS_ENV_VALUE);
