...
use support::{
    ...
    read_server_message, register_runtime_dir, register_spawned_panels_pid,
    unregister_spawned_panels_pid, wait_for_file, wait_for_message_variant, wait_for_socket,
    wait_until,
};
...
fn client_sees_headless_startup_config_diagnostic() {
    ...
    cmd.env("XDG_CONFIG_HOME", &config_home);
    cmd.env("XDG_RUNTIME_DIR", &runtime_dir);
    cmd.env("PANELS_SOCKET_PATH", &api_socket);
    cmd.env_remove("PANELS_CLIENT_SOCKET_PATH");
    cmd.env("SHELL", "/bin/sh");
    cmd.env_remove("PANELS_ENV");
...
fn server_unreachable_shows_clear_error() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_panels"))
        .arg("client")
        .env("PANELS_DISABLE_SOUND", "1")
    ...
    assert!(
        stderr.contains("Is panels server running?"),
