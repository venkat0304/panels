...
### Fixed
- PageUp and PageDown now scroll Panels pane scrollback for normal panes while still forwarding keys to full-screen or mouse-reporting apps.
- Enhanced tilde key sequences now parse correctly, improving compatibility with terminals that emit them.
- `panels integration install codex` now enables the current Codex `[features] hooks = true` flag and migrates the deprecated top-level `codex_hooks` flag.

### Breaking Changes
- `advanced.allow_nested` has moved to `experimental.allow_nested`; update configs that allow nested Panels launches.
...
### Added
- Added manual pane labels through `panels pane rename`, the `pane.rename` socket API, an optional `keys.rename_pane` binding, and the right-click pane menu.
...
- Added `panels integration status [--outdated-only]` so installed agent integrations can be checked for legacy or outdated versions.
...
### Added
- Added ANSI-formatted pane reads to the CLI and socket API with `panels pane read --format ansi` / `--ansi`, preserving colors and styles for visible and recent pane output.
...
- `panels update` now downloads and prepares the new binary before stopping a running server, reducing the chance of interrupting an active session when download or install preparation fails.

## [0.5.6] - 2026-05-09

### Added
- Added the `vesper` built-in theme. (#71, thanks @nexxeln)
- Added `panels --remote <ssh-target>`, so you can use Panels as a thin client for remote servers without SSHing in first. Panels connects over SSH, bootstraps a matching remote `panels` binary when needed, starts the remote server automatically, and streams an efficient terminal view back to your local terminal.
...
### Fixed
- OSC 8 hyperlinks emitted inside panes now remain clickable after Panels renders them, including titled markdown-style links.
...
### Added
- Added a mobile layout for narrow terminals, making it practical to SSH into your machine and run panels from your phone.
...
### Fixed
- Visible active-tab panes that finish while the outer terminal is unfocused are now marked as seen when you return to panels, preventing stale done/attention indicators.
...
### Added
- Added named persistent sessions, so you can keep separate panels environments for different projects or contexts while sharing the same global config. See the docs for the full session CLI. (#57, thanks @fbettag)
- Added `panels status`, `panels status server`, and `panels status client` to inspect the local client, running server, protocol compatibility, socket path, and whether a restart is needed.

### Changed
- Focused panes can now still alert you through terminal notifications when the panels terminal window is unfocused, so active work does not go quiet just because you switched to another app.
...
- `panels integration install` and `panels integration uninstall` now work without requiring a running panels server.
...
### Added
- Config can now be reloaded in the running app/server from the global menu or with `panels server reload-config`, applying safe live settings without restarting the persistent server.
...
- Panels now writes separate capped support logs for app, client, and server modes, making persistent-session issue reports easier to diagnose without unbounded log growth.
...
- In-app toast delivery now stays inside panels instead of also forwarding a terminal/desktop notification.

## [0.5.0] - 2026-04-21

### Breaking Changes Please Read
- panels now defaults to a persistent server/client session model. running `panels` starts or reattaches to a background session server instead of launching the old single-process UI.
- quitting the UI in default mode now detaches the current client and leaves the shared session running. use `panels server stop` to stop the background server explicitly.
- the old monolithic behavior is still available as an escape hatch with `panels --no-session`.

### Added
- Persistent sessions are now the default product behavior. You can detach and reattach without stopping pane processes.
- Added the thin client and headless server as first-class product components, including auto-detect launch, explicit `panels client`, and `panels server stop`.
...
### Breaking Changes Please Read
- The update flow changes in `0.4.11`. Panels no longer installs updates silently in the background. Starting with this release, panels only checks for updates and shows them in the UI. To install a new release, quit panels and then run `panels update` manually in your shell.
- This prepares the upcoming `0.5.0` persistence release. Panels is moving from the old single-binary update model toward a persistent server/client session model, so your workspace can keep running while clients attach, detach, and reconnect.
- The reason for this change is upgrade safety. Panels needs to stop the old running process cleanly before the new client/server model takes over, so manual update avoids mixed-version states during the transition.

### Added
- Hook-reported agent state can now use custom agent labels, so integrations are no longer limited to panels’s built-in agent names. Custom labels now flow through pane/workspace UI and the socket API anywhere agent names are shown.

## [0.4.10] - 2026-04-14

### Added
- Prefix mode now supports custom command keybindings via `[[keys.command]]`, so you can launch detached shell helpers or open temporary overlay panes from inside panels using the active workspace, tab, pane, and cwd context.
...
### Fixed
- Droid's primary-screen redraws no longer erase pane scrollback inside panels, while normal scrollback-clear behavior is preserved elsewhere.
...
### Added
- Themes can now set `panel_bg = "reset"` to let panels’s panel chrome inherit the host terminal background instead of painting an opaque panel fill. This also accepts the aliases `default`, `none`, and `transparent`.
...
### Added
- `panels workspace create` and `panels tab create` now support `--label`, so scripts and agents can name new workspaces and tabs immediately instead of creating them first and renaming them afterward.
- The global menu now includes a manual **reload keybinds** action, so you can apply `config.toml` keybinding changes without restarting panels.
- The socket API and CLI now expose a `done` agent status, including `panels wait agent-status --status done`, so automation can distinguish finished agent runs from panes that are merely idle.

### Changed
- Session state is now saved automatically with a debounce while you work, so recent workspace, tab, pane, and sidebar changes are preserved more reliably even if panels exits unexpectedly.
...
- `panels pane run ...` now uses the bracketed-paste-aware input path, improving compatibility with shells and terminal apps that expect pasted command text to arrive atomically.
...
- `panels pane run ...` now submits the full command atomically in one request, fixing cases where scripted commands did not reliably execute because the final Enter was sent separately.
...
### Major Changes
- Panels now uses a Ghostty-backed terminal engine as its pane runtime.
...
- Debug builds now use separate runtime/config paths from normal releases, which avoids local development sessions colliding with your main panels install.

### Fixed
- Starting a second panels instance against an active socket now fails fast with a clear error instead of clobbering the running session.
...
- Added first-class tab support to the local socket API and CLI wrappers, including `panels tab ...` commands and tab ids like `1:2` alongside workspace-scoped pane ids.
...
- Added a post-update release-notes screen so panels can explain what changed after an update is installed.
...
### Changed
- Mouse wheel handling now follows the tmux/Ghostty model more closely: fullscreen apps receive wheel input when they own scrolling, while panels keeps host scrollback for panes that are behaving like a normal terminal transcript.
- Pane scrollbars now only appear when panels has real host scrollback for that pane, instead of implying a host-managed scroll position for app-owned scrolling.

### Fixed
- Fixed Codex and pi panes becoming unscrollable in panels by preserving recoverable host history for top-anchored normal-screen output, without relying on alternate-screen scrollback retention.
- Fixed pane wheel routing so apps using mouse reporting or alternate-scroll behavior can receive scroll input directly instead of having panels always intercept it.
...
- Fixed alternate-screen scrollback handling so full-screen terminal apps can preserve recoverable history inside panels panes instead of losing rows that scroll off.
- Fixed Codex in panels panes losing transcript/history while running in alternate screen, so past output remains scrollable instead of disappearing as the session grows.
...
### Added
- Panels now checks for updates at startup and periodically while it stays open, so long-running sessions can still discover new releases without a restart cycle.
...
### Added
- Added a local Unix socket API for controlling running panels sessions, including workspace and pane management, pane reads, text/key input, pane splitting, and output waits.
...
- Added CLI wrappers on top of the socket API with `panels workspace ...`, `panels pane ...`, and `panels wait ...`, using compact public ids like `1` and `1-2` for scripting and agent orchestration.
- Added a settings popup with mouse support for changing themes, sound alerts, and toast notifications from inside panels.
...
- Panels now blocks nested launches by default when started inside a panels-managed pane; set `advanced.allow_nested = true` to opt back in.
...
### Documentation
- Refreshed the README, socket API reference, and reusable agent skill docs to better explain panels's agent multiplexer model and integration surface.
...
### Fixed
- Improved foreground job detection on Linux and macOS so panels can recognize agents that run through wrapper processes or generic runtimes, including cases like Codex running under `node`.
