...
panels reads config from:

```text
~/.config/panels/config.toml
```
...
```text
~/.config/panels/session.json
~/.config/panels/sessions/<name>/session.json
```

Use `panels session list`, `panels session attach <name>`, `panels session stop <name>`, and `panels session delete <name>` to inspect and manage named session namespaces. Add `--json` to session commands when scripts need machine-readable output.

In default persistence mode, quitting the UI detaches the current client. Use `panels server stop` to stop the shared background server.

print the full default config with:

```bash
panels --default-config
```

if a config value is invalid, or two navigate actions use the same keybinding, panels falls back to a safe default and shows a startup warning in the UI.
...
You can also use the global menu inside panels and choose `reload config`.
...
If the TOML cannot be read or parsed, reload applies nothing and keeps the current running state. If keybindings are invalid, panels keeps the current keybindings while applying other valid reloadable settings where possible.
...
Commands run through `/bin/sh -lc`. Panels sets the command working directory to the active pane cwd when available and provides context through environment variables:

| variable | value |
|----------|-------|
| `PANELS_SOCKET_PATH` | active panels socket path |
| `PANELS_BIN_PATH` | current panels binary path |
| `PANELS_ACTIVE_WORKSPACE_ID` | active workspace id |
| `PANELS_ACTIVE_TAB_ID` | active tab id |
| `PANELS_ACTIVE_PANE_ID` | focused pane id |
| `PANELS_ACTIVE_PANE_CWD` | focused pane cwd |
...
key = "shift+g"
type = "shell"
command = "notify-send panels 'custom command ran'"
```

## theme

panels ships with 17 built-in color themes. set one in config:
...
for `panel_bg`, you can also use `reset`, `default`, `none`, or `transparent` to stop panels from painting an opaque panel background and instead use the host terminal's default background.
...
| `mouse_capture` | `true` | capture mouse input for Panels's mouse UI; set false to let the terminal handle normal clicks while still forwarding mouse to pane apps that request it |
...
- `system` — ask the os notification service directly. on macos, panels uses `terminal-notifier` when available and falls back to built-in `osascript`. on linux, `system` requires `notify-send`.
...
when `terminal-notifier` is installed, panels tries to focus the hosting terminal when a notification is clicked. click-to-return is supported for detected ghostty, iterm2, wezterm, kitty, alacritty, and terminal.app sessions.

without `terminal-notifier`, panels falls back to built-in `osascript`. this still shows a macos notification, but clicking the notification may focus the apple script runner instead of returning to your terminal.

compatibility note:
- older configs may still use `ui.toast.enabled = true|false`
- panels still reads that legacy key for compatibility
- if you save toast settings from inside panels, it rewrites the setting to `ui.toast.delivery`
...
- inside tmux, panels wraps notification escapes with tmux passthrough
...
| option | default | description |
|--------|---------|-------------|
| `experimental.allow_nested` | `false` | allow launching panels from inside a panels-managed pane |
| `experimental.kitty_graphics` | `false` | enable experimental local Kitty graphics rendering for attached clients |

### nested launches

By default, panels blocks nested launches when `PANELS_ENV=1` is already present.
...
|----------|-------------|
| `PANELS_LOG` | log level filter (default: `panels=info`) |

## logs

panels writes local file logs under:
...
notes:
- `panels.log` is used by monolithic `--no-session` mode and some top-level startup paths
- persistent session mode mainly uses `panels-client.log` and `panels-server.log`
...
- `PANELS_LOG` can increase verbosity when you need a local repro or deeper debugging
